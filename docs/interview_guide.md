...
InferKernelLab 聚焦 LLM decode 阶段的 KV cache 与 attention kernel。我实现了逻辑到物理 block 映射、PyTorch correctness reference、连续 KV 的 SDPA 基线、paged gather + SDPA 基线，以及直接访问 paged KV 的 Triton kernel。项目重点不是声称某个数字普遍更快，而是用统一输入、正确性误差和可复现测量解释不同实现的成本。它和 MiniTrainBench 形成互补：一个展示训练/预训练系统，一个展示推理内存与 GPU kernel 优化。
...
- runtime/scheduler 当前只模拟资源生命周期和 token-budget prefill/decode，不执行模型。这是项目边界，不能描述成可直接 serving 的框架。
